...
import com.example.speechrecificationapp.Speech_Therapist_Activity
...
class UploadActivity : AppCompatActivity()
{
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        mStorageRef = FirebaseStorage.getInstance().getReference("Therapist")
        ...
        upLoadBtn.setOnClickListener {
            if (mUploadTask != null && mUploadTask!!.isInProgress){
                Toast.makeText(this@UploadActivity,"An Upload is Still in Progress",Toast.LENGTH_SHORT).show()
            }
            else{
                uploadFile()
            }
        ...
        if (mImageUri != null)
        {
            ...
            mUploadTask = fileReference.putFile(mImageUri!!)
                ...
                .addOnSuccessListener { taskSnapshot ->
                    ...
                    Toast.makeText(this@UploadActivity,"Therapist data Upload successful",Toast.LENGTH_LONG).show()

                    val upload = Therapist(
                        name = nameEditText!!.text.toString().trim { it <= ' ' },
                        imageUrl = mImageUri.toString(),
                        description =  descriptionEditText!!.text.toString().trim { it <= ' ' },
                        contact =  contactEditText!!.text.toString().trim { it <= ' ' },
                        experiece =  experienceEditText!!.text.toString().trim { it <= ' ' },
                        time =  timeEditText!!.text.toString().trim { it <= ' ' }
                    )
    ...
    {
        startActivity(Intent(this@UploadActivity, Speech_Therapist_Activity::class.java))
    }
}